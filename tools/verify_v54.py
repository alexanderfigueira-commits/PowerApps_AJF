#!/usr/bin/env python3
"""Static checks for v54 against v53: request card order title, description, contacts."""
import json, re, sys, zipfile
import yaml
from paload import PaLoader

PKG = sys.argv[1] if len(sys.argv) > 1 else 'msapp-versions/AV-CD-v54-card-order.msapp'
BASE = sys.argv[2] if len(sys.argv) > 2 else 'msapp-versions/AV-CD-v53-home-sort.msapp'
fails, n = [], 0
...
# ---- v54: request card order title / description / contacts ----
S = 'RequesDetailScreen'
oc = {c['Name']: c for c in walk(old[S])}
nc = {c['Name']: c for c in walk(new[S])}
diff = [(c['Name'], r['Property']) for c in walk(new[S]) for r in c['Rules']
        if rule(oc[c['Name']], r['Property']) != r['InvariantScript']]
check(len(diff) == 8 and {p for _, p in diff} == {'Y'}, f'changed: {diff}')
box = lambda n: tuple(int(rule(nc[n], k)) for k in ('X', 'Y', 'Width', 'Height'))
left = [k for k, c in nc.items() if c['Parent'] == S and rule(c, 'Visible') == 'Not(locDetailsCollapsed)'
        and k not in ('RS_RequestCard', 'RS_DetailsDivider') and (rule(c, 'X') or '').isdigit()
        and int(rule(c, 'X')) < 779 and (rule(c, 'Y') or '').isdigit()]
for i, p in enumerate(left):
    for q in left[i + 1:]:
        x1, y1, w1, h1 = box(p); x2, y2, w2, h2 = box(q)
        if {p, q} == {'RS_DetailsToggle_contrator', 'RS_LblContractor'}:
            continue  # same 8 px overlap as in v53: the label box is wider than its text
        check(not (x1 < x2 + w2 and x2 < x1 + w1 and y1 < y2 + h2 and y2 < y1 + h1), f'{p} overlaps {q}')
order = [box(k)[1] for k in ('RS_Title', 'RS_LblDescription', 'RS_Description', 'RS_LblOwner', 'RS_Owner')]
check(order == sorted(order), f'order title < description < contacts: {order}')
cy, ch = box('RS_RequestCard')[1], box('RS_RequestCard')[3]
for nm in ('RS_Owner', 'RS_Contractor', 'RS_Description'):
    check(box(nm)[1] + box(nm)[3] <= cy + ch, f'{nm} below the card')
