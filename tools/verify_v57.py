#!/usr/bin/env python3
"""Static checks for v57 against v56: Dashboard nav buttons open the role dashboard."""
import json, re, sys, zipfile
import yaml
from paload import PaLoader

PKG = sys.argv[1] if len(sys.argv) > 1 else 'msapp-versions/AV-CD-v57-dashboard-nav.msapp'
BASE = sys.argv[2] if len(sys.argv) > 2 else 'msapp-versions/AV-CD-v56-role-dashboards.msapp'
fails, n = [], 0
...
# ---- v57: Dashboard nav buttons by role ----
hits = []
    ...
    for c in walk(tp):
        for r in c['Rules']:
            if rule(oc[c['Name']], r['Property']) != r['InvariantScript']:
                hits.append((scr, c['Name'], r['Property']))
        if c['Template']['Name'] == 'button' and (rule(c, 'Text') or '').strip() == '"📊" & Char(10) & "Dashboard"':
            t = rule(c, 'OnSelect')
            check("Navigate('Dashboard-Ope-Administrator')" in t and "Navigate('Dashboard-Ope-Requestor')" in t
                  and 'varUserRole = "ADMINISTRATOR"' in t and cat(c, 'OnSelect') == 'Behavior', f'{scr}.{c["Name"]}')
        for r in c['Rules']:
            check('Navigate(DashboardScreen' not in r['InvariantScript'], f'{scr}.{c["Name"]} still opens DashboardScreen')
check(len(hits) == 15 and {h[2] for h in hits} == {'OnSelect'} and len({h[0] for h in hits}) == 15, f'changed: {hits}')
