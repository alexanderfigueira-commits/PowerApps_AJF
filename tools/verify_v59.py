#!/usr/bin/env python3
"""Static checks for v59 against v58: AI check text, CM_BtnNext, CL_Case default, PublicationChannel."""
import json, re, sys, zipfile
import yaml
from paload import PaLoader

PKG = sys.argv[1] if len(sys.argv) > 1 else 'msapp-versions/AV-CD-v59-fixes.msapp'
BASE = sys.argv[2] if len(sys.argv) > 2 else 'msapp-versions/AV-CD-v58-print-one-sheet.msapp'
fails, n = [], 0
...
# ---- v59: four fixes ----
check(changed == ['ChildLegalScreen', 'ChildMetaScreen', 'ChildValidScreen', 'RequesDetailScreen'], f'changed: {changed}')
diff = []
for scr in changed:
...
check(sorted(diff) == sorted([
    ('RequesDetailScreen', 'RS_AICheck', 'Text'), ('RequesDetailScreen', 'RS_AICheck', 'DisplayMode'),
    ('RequesDetailScreen', 'RS_AICheck', 'Color'), ('RequesDetailScreen', 'RequesDetailScreen', 'OnVisible'),
    ('ChildMetaScreen', 'CM_BtnNext', 'ZIndex'), ('ChildLegalScreen', 'ChildLegalScreen', 'OnVisible'),
    ('ChildValidScreen', 'CV_BtnSaveArchive', 'OnSelect')]), f'changed rules: {diff}')
rs = {c['Name']: c for c in walk(new['RequesDetailScreen'])}
check('"Please select the production type!"' in rule(rs['RS_AICheck'], 'Text'), 'AI check text')
cm = {c['Name']: c for c in new['ChildMetaScreen']['Children']}
zn = int(rule(cm['CM_BtnNext'], 'ZIndex'))
check(zn > int(rule(cm['CM_BottomBar'], 'ZIndex')) and zn < int(rule(cm['CM_AttachShade'], 'ZIndex')), 'CM_BtnNext z-order')
ov = rule(new['ChildLegalScreen'], 'OnVisible')
check('Set(varChildContractCase, "Case 1 - framework / direct service contract")' in ov, 'CL default stored')
cl = {c['Name']: c for c in walk(new['ChildLegalScreen'])}
check('"Case 1 - framework / direct service contract"' in rule(cl['CL_Case'], 'Items'), 'CL_Case first item')
sv = rule({c['Name']: c for c in walk(new['ChildValidScreen'])}['CV_BtnSaveArchive'], 'OnSelect')
check('PublicationChannel: ForAll(' not in sv and 'PublicationChannel: If(' in sv, 'PublicationChannel record')
check(sv.count('(') == sv.count(')') and sv.count('{') == sv.count('}'), 'save brackets')
for scr, tp in new.items():
    for c in walk(tp):
        for r in c['Rules']:
            check('Concat(PublicationChannel' not in r['InvariantScript'], f'{scr}.{c["Name"]} reads PublicationChannel as a table')
