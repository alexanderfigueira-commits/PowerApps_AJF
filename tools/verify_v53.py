#!/usr/bin/env python3
"""Static checks for v53 against v52: HomeGallery sort follows the status chip."""
import json, re, sys, zipfile
import yaml
from paload import PaLoader

PKG = sys.argv[1] if len(sys.argv) > 1 else 'msapp-versions/AV-CD-v53-home-sort.msapp'
BASE = sys.argv[2] if len(sys.argv) > 2 else 'msapp-versions/AV-CD-v52-home-rules.msapp'
fails, n = [], 0
...
# ---- v53: sort follows the status chip ----
...
check(diff == [('HomeGallery', 'Items')], f'changed: {diff}')
it = rule({c['Name']: c for c in walk(new[S])}['HomeGallery'], 'Items')
check(it.rstrip().endswith('If(varFilter = "All", SortOrder.Descending, SortOrder.Ascending)\n)'), 'sort order rule')
check(it.count('(') == it.count(')'), 'brackets')
old_it = rule(oc['HomeGallery'], 'Items')
check(it.split('    Created,')[0] == old_it.split('    Created,')[0], 'filter part unchanged')
