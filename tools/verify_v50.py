#!/usr/bin/env python3
"""Static checks for v50 against v49: plain text for rich-text columns on the print screens."""
import json, re, sys, zipfile
import yaml
from paload import PaLoader

PKG = sys.argv[1] if len(sys.argv) > 1 else 'msapp-versions/AV-CD-v50-print-plaintext.msapp'
BASE = sys.argv[2] if len(sys.argv) > 2 else 'msapp-versions/AV-CD-v49-assign-notify.msapp'
fails, n = [], 0
...
# ---- v50: rich-text columns shown as plain text on the print screens ----
PRINT = ('PrintVideoDetailScreen', 'PrintPodcastDetailScreen', 'PrintPhotoDetailScreen')
diff = []
for s_ in PRINT:
    oc = {c['Name']: c for c in walk(old[s_])}
    for c in walk(new[s_]):
        for r in c['Rules']:
            if rule(oc[c['Name']], r['Property']) != r['InvariantScript']:
                diff.append((s_, c['Name'], r['Property']))
            t = r['InvariantScript']
            for m in re.finditer(r'((?:ThisItem|First\(colPrintReqMedia\))\.(?:Description|Notes|PhotoCaption))', t):
                check(t[:m.start()].endswith('PlainText('), f'{s_}.{c["Name"]}.{r["Property"]}: {m.group(1)} not wrapped in PlainText')
check(len(diff) == 14, f'{len(diff)} rules changed, expected 14')
check({p for _, _, p in diff} == {'Text', 'Color'}, 'only Text and Color may change')
