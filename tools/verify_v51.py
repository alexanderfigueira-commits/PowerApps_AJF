#!/usr/bin/env python3
"""Static checks for v51 against v50: print header values drawn above the header card."""
import json, re, sys, zipfile
import yaml
from paload import PaLoader

PKG = sys.argv[1] if len(sys.argv) > 1 else 'msapp-versions/AV-CD-v51-print-header.msapp'
BASE = sys.argv[2] if len(sys.argv) > 2 else 'msapp-versions/AV-CD-v50-print-plaintext.msapp'
fails, n = [], 0
...
check(changed == sorted(PRINT_SCREENS), f'unexpected screens changed: {changed}')
...
# ---- v51: header values drawn above the header card ----
PRINT = {'PVD': 'PrintVideoDetailScreen', 'PPoD': 'PrintPodcastDetailScreen', 'PPD': 'PrintPhotoDetailScreen'}
diff = []
for p, s_ in PRINT.items():
    oc = {c['Name']: c for c in walk(old[s_])}
    nc = {c['Name']: c for c in walk(new[s_])}
    for c in nc.values():
        for r in c['Rules']:
            if rule(oc[c['Name']], r['Property']) != r['InvariantScript']:
                diff.append((s_, c['Name'], r['Property']))
    cz = int(rule(nc[f'{p}_HeaderCard'], 'ZIndex'))
    for k in ['Title', 'Badge'] + [f'HdrLbl{i}' for i in range(7)] + [f'HdrVal{i}' for i in range(7)]:
        check(int(rule(nc[f'{p}_{k}'], 'ZIndex')) > cz, f'{p}_{k} is under the header card')
check(len(diff) == 27 and {d[2] for d in diff} == {'ZIndex'}, f'changed rules: {len(diff)} {sorted({d[2] for d in diff})}')
